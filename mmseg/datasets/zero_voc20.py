...
class ZeroPascalVOCDataset20(CustomDataset):
    ...
    def evaluate(self,
        ...
        print('\n' + '+++++++++++ Average Metric +++++++++++++')
        if float(mIoU_seen)==0.0 or float(mIoU_unseen)==0.0:
            hIoU = 'nan'
        else:
            hIoU = round(2*mIoU_seen*mIoU_unseen/(mIoU_seen+mIoU_unseen),2) #round(2/(1/mIoU_seen+1/mIoU_unseen),2)
        Average_summary_table_data = PrettyTable()
        Average_summary_table_data.add_column('pAcc', [pAcc])
        Average_summary_table_data.add_column('mIoU(S)', [mIoU_seen])
        Average_summary_table_data.add_column('mIoU(U)', [mIoU_unseen])
        Average_summary_table_data.add_column('hIoU', [hIoU])
        print_log('Average Summary:', logger)
        print_log(Average_summary_table_data.get_string(), logger=logger)
