...
class ATMSingleHeadSeg(BaseDecodeHead):
    ...
    def forward(self, inputs_both, self_training=None):
        ...
        text_embeddings = []
        text_embeddings.append(text_token)
        out = {}
        x = []
        ...
        lateral = laterals[-1]
        prompt_pred = self.forward_mask(lateral,cls_token,text_embeddings[0])
        out.update({"pred_masks":prompt_pred})
    ...
    def forward_mask(self,lateral,cls_token,text_feat):
        ...
        return pred
        # if i ==0:
        #     # if pred.shape[1]>len(self.base_class):
        #     #     out.update({"pred_masks": pred[:,:-1,:]}) # b,c',32,32
        #     #     out.update({"pred_bg_masks": pred[:,-1,:]}) # b,1,32,32
        #     # else:
        #     out.update({"pred_masks": pred})
        #     # continue
        # else:
        #     out.update({"prompt_pred_masks":pred})
