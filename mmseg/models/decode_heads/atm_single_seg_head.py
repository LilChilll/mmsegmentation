...
class ATMSingleHeadSeg(BaseDecodeHead):
    def __init__(
        ...
        self.start_contrastive_learning = False
    ...
    def forward_train(self, inputs,img_metas, gt_semantic_seg, train_cfg, self_training=False, st_mask=None):
        ...
        if self_training:
            pseudo_semantic_masks = seg_logits['pred_masks'].clone().detach() # b,c,h,w
            pseudo_semantic_masks = pseudo_semantic_masks.sigmoid()
            pseudo_semantic_masks[:, self.base_class, :, :] = -1
    ...
    def forward(self, inputs_both, self_training=None):
        inputs = tuple([inputs_both[0]]) # visual feature (b,512,32,32)
        cls_token = inputs_both[1] #[1] # cls token (4,512)
        text_embedding = inputs_both[2] # c',512
        # if len(inputs_both)>3:
        #     self.start_contrastive_learning = True
        #     prompt_text_embedding = inputs_both[3] # b,c',512
        out = {}
        x = []
        ...
        lateral = laterals[-1]
        pred_masks = self.forward_mask(lateral,cls_token,text_embedding)
        out.update({"pred_masks":pred_masks})
        # if self.start_contrastive_learning:
        #     feature_map = x[-1].permute(0, 2, 1).reshape(bs, -1, pred_masks_ori.size()[-2], pred_masks_ori.size()[-1]).contiguous()
        #     visual_features = self.reconstruct_visual_features(feature_map, pred_masks_ori)
        #     out.update({"feature_pairs":(visual_features,prompt_text_embedding)})
        if not self.training:
            if self_training:
                out["pred"] = self.semantic_inference(out, self.base_class) #(bs, 20, 224, 224)
                
            else:
                out["pred"] = self.semantic_inference(out,self.base_class,weight=0.1)
            return out["pred"]                  
        return out
    ...
    def reconstruct_svisual_features(self,feature_map, mask_pred):
        """
        重构类别级别视觉特征
    ...
    def semantic_inference(self, out,base_class, weight=0.0):
        
        mask_pred = out["pred_masks"]
        mask_pred = mask_pred.sigmoid()
        # mask_pred= out.sigmoid()
        mask_pred[:,base_class] = mask_pred[:,base_class] - weight
        return mask_pred
